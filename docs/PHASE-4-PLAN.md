...
| | **4.D scope adjusted post-PDF reading (commit `a4bfc77`+):** Original list (Burns extension, Pyoderma gangrenosum, Necrobiosis lipoidica, Skin grafts) replaced with one new umbrella indication (Inflammatory Dermatoses, Level C) plus dermatology department activation. Three of four original indications were already in the FDA-approved set (chronic wounds → Enhancement of Healing in Selected Problem Wounds; radiation injury → Delayed Radiation Injury; skin grafts → Compromised Skin Grafts and Flaps; burns → covered by Necrotizing Soft Tissue Infections). Necrobiosis lipoidica is not in the source PDF. **Source PDF anchors scope; plan reflected pre-PDF estimation.** Same source-PDF scope check is to be applied at the start of 4.E and 4.F. | revised | |

...
## Current phase
**Phase 4.B — Long COVID indication.** Complete and merged to main (PR #1, merge commit `c276fd7` at 2026-04-29 05:58:50Z). Per Phase 4 plan, next is **4.C Fibromyalgia** — second content cluster, second `tier=emerging` entry.

**Live preview:** https://hbot-resource.pages.dev (HTTPS, `noindex` during build phase). **107 routes** deployed (was 105 → +2 for `/indications/long-covid/` × 2 locales). Indications taxonomy now tier-stratified: FDA-Approved Indications (14) + Emerging Evidence (1, growing through Phase 4). DOI verifier wired into Cloudflare build via `prebuild` hook — broken DOIs fail the deploy. **Greek terminology register locked at `docs/research/el-glossary.md`** — binding for all subsequent Phase 4 clusters.

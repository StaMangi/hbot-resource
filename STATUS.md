...
## Current phase
**Phase 4 COMPLETE.** All 5 content clusters (4.B–4.F) + 3 standalone pages (4.G safety, 4.H programme design, 4.I operator blueprint) + final QA (4.J) shipped. Awaiting Stamos's single comprehensive review on `phase-4/run-e-to-j` PR before merge to main.

**Live preview:** https://hbot-resource.pages.dev (HTTPS, `noindex` during build phase). **115 routes** deployed. Indications taxonomy: FDA-Approved Indications (14) + Emerging Evidence (5 — Long COVID, Fibromyalgia, Inflammatory Dermatoses, Aesthetic & Reconstructive Recovery, Endometriosis). 19 indications total. **44 references** in collection. **All 12 departments now have populated applications** (3 new in Phase 4: dermatology, plastic-surgery, women's-health). DOI verifier wired into Cloudflare build via `prebuild` hook. **Greek terminology register locked at `docs/research/el-glossary.md`** — full Phase 4 register, ~120 EN→EL term pairs across §§ 1–7 (study types, evidence levels, caveat register, symptom vocabulary, treatment/protocol, trial names, plus per-cluster additions § 4.C/4.D/4.E/4.F). Protocol vocabulary fully bilingual via 4.D schema migration. Three new standalone pages (`/safety/`, `/programme-design/`, `/operator-blueprint/`) populated bilingual EN+EL with cited claims only. **Cumulative Phase 4 editorial catches: 11** (6 inherited integrity issues from 4.B · 0 from 4.C · 1 plan-vs-PDF mismatch in 4.D · 2 EL parity fixes in 4.D · 1 plan-vs-PDF mismatch in 4.E · 1 unsupported pilot-study claim caught in 4.F).

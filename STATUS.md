...
## Current phase
**Phase 4.D — Inflammatory Dermatoses indication + Dermatology dept activation.** Complete and merged to main (PR #4, merge commit `68a5f8e`). Per Phase 4 plan, next is **4.E Plastic Surgery** — same source-PDF scope check pattern set in 4.D to be applied at the start.

**Live preview:** https://hbot-resource.pages.dev (HTTPS, `noindex` during build phase). **111 routes** deployed (was 109 → +2 for `/indications/inflammatory-dermatoses/` × 2 locales). Indications taxonomy: FDA-Approved Indications (14) + Emerging Evidence (3 — Long COVID + Fibromyalgia + Inflammatory Dermatoses). 36 references in collection (was 32 + 4 dermatology). Dermatology department now activated with 5 applications (4 cross-referencing existing FDA indications + 1 pointing to Inflammatory Dermatoses). DOI verifier wired into Cloudflare build via `prebuild` hook. **Greek terminology register locked at `docs/research/el-glossary.md`** — refined by 4.C and 4.D; protocol vocabulary fully bilingual via 4.D schema migration (sessions + frequency now `bilingual` in the schema). **Cumulative Phase 4 editorial catches: 9** (6 inherited integrity issues from 4.B · 0 from 4.C · 1 plan-vs-PDF mismatch in 4.D · 2 EL parity fixes in 4.D).

...
## Current phase
**Phase 4.A — DOI verification tooling + 3 new dept entries.** Complete and deployed. Per Phase 4 plan, next is **4.B Long COVID** (first content cluster — extra-careful review pause point per amendment).

**Live preview:** https://hbot-resource.pages.dev (HTTPS, `noindex` during build phase). **105 routes** deployed (was 99 → +6 for 3 new dept detail pages × 2 locales). DOI verifier wired into Cloudflare build via `prebuild` hook — broken DOIs fail the deploy.
